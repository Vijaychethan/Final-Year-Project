...
def main():
    ...
    selected_option = st.selectbox("Select an option", ["Small", "Medium", "Large"])
    options_to_images = {
        "Small": "BluePrints\small.png",
        "Medium": "BluePrints\medium.png",
        "Large": "BluePrints\large.png"
    }
    
    # Get the image path based on the selected option
    image_path = options_to_images.get(selected_option, None)
...
def Predict(pil_image, class_name):
    ...
    person_mask = get_mask(person_results, class_id=0, color=(0, 0, 255))
    return person_mask
...
def capture_image(frame, blueprint_img):
    ...
    class_name = "person"  # Specify the class you want to segment
